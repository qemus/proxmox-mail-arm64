...
function append_local_crate_patches() {
	cargo_toml=${1}
	local_root=${2}
	...
	if [ ! -d "${local_root}" ]; then
		echo "Error: local crate root not found: ${local_root}" >&2
		return 1
	fi
	...
	while IFS= read -r crate_toml; do
		...
		rel_path=$(realpath --relative-to="$(dirname "${cargo_toml}")" "${crate_dir}")
...
DEB
    ...
    # The blank line and [profile.release] are important: the original rules append
    # lto/opt-level/codegen-units immediately afterwards, and they must not end up
    ...
    # Instead of hardcoding every crate name, patch every Cargo package found in
    # the checked-out proxmox.git tree. This keeps the build working when PDM adds
    # or removes Proxmox-local Rust crates in the future.
    append_local_crate_patches Cargo.toml "${SOURCES}/proxmox"

    echo "Local UI [patch.crates-io] entries:"
    awk '/^\[patch\.crates-io\]$/ { show=1; print; next } /^\[/ && show { show=0 } show { print }' Cargo.toml
